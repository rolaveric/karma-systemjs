...
var pathOrNpm = function(systemjsConfig, basePath, module, npmPath) {
	var path = systemjsConfig.paths[module];
	if (path) {
		return basePath + systemjsConfig.baseURL + path;
	} else {
		return getDependencyPath(module, npmPath);
	}
};
...
var initSystemjs = function (config) {
	...
	var es6ModuleLoaderPath = pathOrNpm(kSystemjsConfig.config, basePath, 'es6-module-loader', '/dist/es6-module-loader.src.js');
	var systemjsPath = pathOrNpm(kSystemjsConfig.config, basePath, 'systemjs', '/system.src.js');
